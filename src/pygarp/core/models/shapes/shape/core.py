...
class Shape(DiscretizationMixin, TransformationMixin):
    def __init__(
        ...
        if _skip:
            points_array = points
            origin_array = origin if origin is not None else np.zeros(2, dtype=np.float64)
        else:
            points_array: npt.NDArray[np.float64] = validate_array_of_nd_coordinates(points, 2)
            origin_array: npt.NDArray[np.float64] = validate_nd_coordinates(origin, 2) if origin is not None else np.zeros(2, dtype=np.float64)

        if not assume_sort:
            points_array = sort_by_tolerance_2d_array(points_array, decimals=abs(int(np.log10(eps))))
    ...
    def barycenter(self) -> npt.NDArray[np.float64]:
        if self._barycenter is None:
            self._barycenter = np.array([self._polygon.centroid.x, self._polygon.centroid.y], dtype=np.float64)
    ...
    def __str__(self) -> str:
        ...
        ident = f"'{self.name}'" if self.name else (f"ID:{self.id}" if self.id else "Unnamed")
    ...
    def __repr__(self) -> str:
        ...
        n_vertices = len(self.polygon.exterior.coords) - 1
        b = self.bounds
        bounds_str = f"({b[0]:.1f}, {b[1]:.1f}, {b[2]:.1f}, {b[3]:.1f})" if b else "None"
        orig = self.origin
