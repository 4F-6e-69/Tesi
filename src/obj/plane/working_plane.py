...
class WorkingSpace:
    ...
    def is_valid(self) -> bool:
        ...
        if not np.allclose(check_matrix, self.space_matrix, self.tolerance) or not np.allclose(check_traspose,
        ...
        if not np.isclose(np.linalg.det(self.space_matrix), 1, WorkingSpace.EPS_10):
            warnings.warn(
